...
#     # Use CRF tagger — same interface, better accuracy
#     from src.crf_pos_tagger import CRFMalayalamPOSTagger
#     crf_model_path = os.path.join(os.path.dirname(os.path.dirname(
#         os.path.abspath(__file__))), 'data', 'crf_pos_model.pkl')
#     if os.path.exists(crf_model_path):
#         tagger = CRFMalayalamPOSTagger(mode='crf_pretrained', model_path=crf_model_path)
#         print(f"  Using trained CRF POS tagger")
#     else:
#         tagger = CRFMalayalamPOSTagger(mode='rule')
#         print(f"  Using rule-based POS tagger (no CRF model found at {crf_model_path})")
#     processed = []
...
def run_preprocessing():
    ...
    # POS tagger selection (priority order):
    #   1. CRF++ pre-tagged cache (crfpp_tags.pkl) — best quality, from real CRF++ model
    #   2. sklearn-crfsuite model (crf_pos_model.pkl) — good, trained on pseudo-tags
    #   3. Rule-based heuristic — fallback, no dependencies
    from src.crf_pos_tagger import CRFMalayalamPOSTagger
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    crfpp_cache    = os.path.join(base_dir, 'data', 'crfpp_tags.pkl')
    sklearn_model  = os.path.join(base_dir, 'data', 'crf_pos_model.pkl')

    crfpp_tag_cache = None
    if os.path.exists(crfpp_cache):
        with open(crfpp_cache, 'rb') as _f:
            crfpp_tag_cache = pickle.load(_f)
        tagger = CRFMalayalamPOSTagger(mode='rule')  # fallback for unseen tokens
        print(f"  Using CRF++ pre-tagged cache ({len(crfpp_tag_cache)} sentences)")
    elif os.path.exists(sklearn_model):
        tagger = CRFMalayalamPOSTagger(mode='crf_pretrained', model_path=sklearn_model)
        print(f"  Using sklearn-CRF POS tagger")
    else:
        tagger = CRFMalayalamPOSTagger(mode='rule')
        print(f"  Using rule-based POS tagger")
    processed = []
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        src_tokens_raw = tokenize(clean_text(src_text))
        cache_key = tuple(src_tokens_raw)
        if crfpp_tag_cache and cache_key in crfpp_tag_cache:
            src_tokens = src_tokens_raw
            src_tags   = crfpp_tag_cache[cache_key]
        else:
            src_tokens, src_tags = process_text(src_text, tagger)
        if len(src_tokens) == 0:
            continue
